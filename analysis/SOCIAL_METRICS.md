...
This legacy script adds two extension metrics:
...
The former Conversation Embedding Dispersion (CED) implementation has been removed. The
canonical conversation-level diversity analysis is now the sequence-aware semantic-route
metric in `analysis/semantic_routes.py`.
...
```bash
py -3 analysis/social_metrics.py --data-dir data/generated_v2 --n-sb 50 --embedding-backend sentence-transformers --embedding-model all-MiniLM-L6-v2
```
...
## Conversation-Level Diversity: Superseded

The old CED reduced each complete conversation to one mean-pooled vector. That discarded the
order in which topics and ideas developed, and its optional TF-IDF fallback was dominated by
word and topic overlap. It is no longer implemented by this script, and its old `ced_*`
outputs must not be interpreted as current results.

Use `analysis/semantic_routes.py` for the replacement analysis. It preserves semantic
development across normalized conversation progress, compares conversations only within the
same topic, and measures whether each conversation has a highly similar route sibling.
...
5. Should CAS use TF-IDF for speed or sentence-transformers for better semantic meaning?
...
4. Decide whether CAS is useful enough to retain as a supporting diagnostic; it is not the
